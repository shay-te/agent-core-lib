...
def load_config():
    if not AgentInstance.config:
        path = os.path.join(os.path.dirname(__file__), '..', 'data')
        load_dotenv(dotenv_path=os.path.join(path, '.env'))

        GlobalHydra.instance().clear()
        hydra.initialize(config_path=os.path.join('../../../../ob-love-admin-backend', 'data', 'config'), caller_stack_depth=1)
        AgentInstance.config = hydra.compose('config.yaml')
    return AgentInstance.config


def sync_create_start_core_lib() -> AgentCoreLibClass:
    threadLock.acquire()
    if not AgentInstance.instance:
        try:
            ...
            config = load_config()
            AgentInstance.instance = AgentCoreLibClass(config)
            AgentInstance.instance.install(config)
            AgentInstance.instance.start_core_lib()
